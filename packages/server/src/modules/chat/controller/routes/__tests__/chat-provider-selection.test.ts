...
describe("chat provider selection", () => {
  ...
  beforeEach(async () => {
    ...
    const busModule = await import("@/bus/index");
    ...
    const { getProviderRuntime, resetProviderRuntimeForTests } = await import("@/provider/runtime");
  ...
  it("returns 401 when explicit provider is unauthenticated", async () => {
    const chatRouter = (await import("../chat.route")).default;
