...
### Phase 8: Hybrid Agent (Smart Context Management)
...
├── src/
│   ├── agents/
│   │   ├── zai-hybrid-agent.ts          # LanguageModelV3 implementation
│   │   ├── intent-classifier.ts         # Intent detection logic
│   │   ├── specialized-prompts.ts       # Prompts from MCP server (sync)
...
#### Specialized Prompts Mapping

| Intent Type                  | System Prompt                      | Output Format                 |
| ---------------------------- | ---------------------------------- | ----------------------------- |
| `ui-to-artifact/code`        | UI_TO_ARTIFACT_PROMPTS.code        | Frontend code (HTML/CSS/JS)   |
| `ui-to-artifact/prompt`      | UI_TO_ARTIFACT_PROMPTS.prompt      | AI prompt for recreation      |
| `ui-to-artifact/spec`        | UI_TO_ARTIFACT_PROMPTS.spec        | Design specification document |
| `ui-to-artifact/description` | UI_TO_ARTIFACT_PROMPTS.description | Natural language description  |
| `error-diagnosis`            | ERROR_DIAGNOSIS_PROMPT             | Root cause + solutions        |
| `diagram-analysis`           | DIAGRAM_UNDERSTANDING_PROMPT       | Diagram structure explanation |
| `data-viz`                   | DATA_VIZ_ANALYSIS_PROMPT           | Data insights and trends      |
| `ui-diff`                    | UI_DIFF_CHECK_PROMPT               | Visual differences            |
| `text-extraction`            | TEXT_EXTRACTION_PROMPT             | Extracted text content        |
| `general-image`              | GENERAL_IMAGE_ANALYSIS_PROMPT      | Flexible (adapts to request)  |

#### Implementation Details

**1. Agent implements LanguageModelV3 (`zai-hybrid-agent.ts`)**

```typescript
export class ZaiHybridAgent implements LanguageModelV3 {
  readonly specificationVersion = "v3";
  readonly modelId: string;

  private textModel: LanguageModelV3;
  private visionModel: LanguageModelV3;
  private specializedPrompts: SpecializedPrompts;
  private intentClassifier: IntentClassifier;

  constructor(options: ZaiHybridAgentOptions) {
    const provider = createZai(options);
    this.modelId = options.textModelId ?? "glm-4.7";
    this.textModel = provider(this.modelId);
    this.visionModel = provider(options.visionModelId ?? "glm-4.6v");
    this.specializedPrompts = loadSpecializedPrompts(); // sync
  ...
  async doGenerate(options: LanguageModelV3CallOptions): Promise<LanguageModelV3GenerateResult> {
    const { images, userText, hasImages } = extractImagesAndText(options.prompt);
...
function selectVisionStrategy(intent: Intent, userText: string): "multi" | "per-image" {
  if (intent.type === "ui-diff" || /compare|difference|before\/after/i.test(userText)) {
    return "multi";
  }
  return "per-image";
}
...
export class VisionRequestHandler {
  ...
  async execute(request: VisionRequest): Promise<string> {
    const result = await this.visionModel.doGenerate({
      prompt: buildVisionPrompt(request, this.specializedPrompts),
    });
...
export interface Intent {
  type:
    | "ui-to-artifact/code"
    | "ui-to-artifact/prompt"
    | "ui-to-artifact/spec"
    | "ui-to-artifact/description"
    | "error-diagnosis"
    | "diagram-analysis"
    | "data-viz"
    | "ui-diff"
...
```typescript
import { createZaiAgent } from "ekacode";

const agent = createZaiAgent({
  apiKey: process.env.ZAI_API_KEY,
  textModelId: "glm-4.7",
...
// packages/ekacode/src/agents/index.ts
export { createZaiAgent, ZaiHybridAgent } from "./zai-hybrid-agent";
export { IntentClassifier } from "./intent-classifier";
export type {
  ZaiHybridAgentOptions,
  Intent,
  VisionRequest,
  VisionImage,
  SpecializedPrompts,
...
- [ ] `specialized-prompts.ts` - Prompt registry
